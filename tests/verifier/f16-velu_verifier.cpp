...
// Forward declarations for f16 velu kernels
extern "C" {
...
int main() {
  ...
  // f16-velu: Exponential Linear Unit for float16
  // For x >= 0: output = x * beta
  // For x < 0:  output = alpha * (exp(x * prescale) - 1)
  // Both implementations use polynomial approximation with range reduction (rr1 p3)
  ...
  // Setup params struct with typical ELU parameters
  // prescale = 1.0, alpha = 1.0, beta = 1.0 (standard ELU)
  struct xnn_f16_elu_params params;
  params.scalar.prescale = xnn_float16_from_float(1.0f);
  params.scalar.alpha = xnn_float16_from_float(1.0f);
  params.scalar.beta = xnn_float16_from_float(1.0f);
  ...
  // NEON loads as uint16 and reinterprets to f16, RVV loads directly as f16
  ...
  // Symbolic input elements (as bitvectors and fp16)
  ...
  for (size_t i = 0; i < num_elements; i++) {
    ...
    // Constrain input range to avoid extreme values that cause numerical issues
    // The sat_cutoff is about -8.3125, so inputs below that are clamped
    // Keep inputs in reasonable range for ELU polynomial approximation
    Term ten = tm.mkFloatingPoint(5, 11, tm.mkBitVector(16, 0x4900));  // 10.0h
    Term neg_eight = tm.mkFloatingPoint(5, 11, tm.mkBitVector(16, 0xC800));  // -8.0h
    Term in_range_upper = tm.mkTerm(Kind::FLOATINGPOINT_LEQ, {x_fp, ten});
    Term in_range_lower = tm.mkTerm(Kind::FLOATINGPOINT_GEQ, {x_fp, neg_eight});
    solver.assertFormula(in_range_upper);
    solver.assertFormula(in_range_lower);
  }
  ...
  std::cout << "Testing XNNPACK f16_velu: NEON vs RISC-V equivalence" << std::endl;
  std::cout << "Operation: ELU (Exponential Linear Unit)" << std::endl;
  std::cout << "  x >= 0: output = x * beta" << std::endl;
  std::cout << "  x < 0:  output = alpha * (exp(x * prescale) - 1)" << std::endl;
  std::cout << "Parameters: prescale=1.0, alpha=1.0, beta=1.0" << std::endl;
  ...
  xnn_f16_velu_ukernel__rvv_rr1_p3(
      batch,
      reinterpret_cast<const _Float16*>(input_riscv.data()),
      reinterpret_cast<_Float16*>(output_riscv.data()),
      &params);

  // Collect NEON output results from memory
  // NEON code stores via vst1q_u16 (full 8-element vectors)
