...
int main() {
  ...
  // Setup params struct with test slope value
  struct xnn_f32_lrelu_params params;
  params.scalar.slope = 0.25f;  // Common leaky ReLU slope
  ...
  for (size_t i = 0; i < num_elements; i++) {
    ...
    // Vector memory for vle32 loads (using m1 which can hold up to 4 elements typically)
    std::vector<Term> chunk;
    for (size_t j = 0; j < 8; j++) {  // m1 can hold varying elements depending on VLEN
      size_t idx = i + j;
      if (idx < num_elements) {
  ...
  std::cout << "Testing XNNPACK f32_vlrelu: NEON vs RISC-V equivalence" << std::endl;
  std::cout << "Operation: output[i] = input[i] >= 0 ? input[i] : input[i] * slope" << std::endl;
  std::cout << "Number of elements: " << num_elements << std::endl;
  std::cout << "Batch size (bytes): " << batch << std::endl;
  std::cout << "Slope: " << params.scalar.slope << std::endl;
  ...
  xnn_f32_vlrelu_ukernel__rvv_u8(
      batch,
      input.data(),
      output_riscv.data(),
      &params);
