...
int main() {
  ...
  // Allocate arrays
  float input_a[batch] = {0};
  float input_b[batch] = {0};
  ...
  std::vector<Term> symbolic_a = createSymbolicArrayFP32(tm, batch, "a");
  std::vector<Term> symbolic_b = createSymbolicArrayFP32(tm, batch, "b");

  // Populate NEON memory
  SymbolicNEONHelpers::populateMemoryF32(input_a, symbolic_a);
  SymbolicNEONHelpers::populateMemoryF32(input_b, symbolic_b);

  // Populate RISC-V memory
  SymbolicRISCVHelpers::populateMemoryF32(input_a, symbolic_a);
  SymbolicRISCVHelpers::populateMemoryF32(input_b, symbolic_b);


  // Default params (unused but required by interface)
  struct xnn_f32_default_params params;
  (void)params;

  std::cout << "Testing XNNPACK f32-vbinary: NEON vs RISC-V equivalence" << std::endl;
  ...
  xnn_f32_vmulc_ukernel__neon_u4(batch * sizeof(float), input_a,
                        input_b, output_neon, &params);

  // Run RISC-V implementation
  std::cout << "Running RISC-V implementation..." << std::endl;
  xnn_f32_vmulc_ukernel__rvv_u4(batch * sizeof(float), input_a,
  ...
  for (size_t i = 0; i < compare_count; i++) {
    ...
    // Denormal/zero handling for flush-to-zero differences
