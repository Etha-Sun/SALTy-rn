...
int main() {
  ...
  // f32-vrndd: Round toward negative infinity (floor)
  // output[i] = floor(input[i])
  // batch is in bytes, so batch/sizeof(float) is the number of elements

  const size_t num_elements = 9;  // 8 elements to match the u8 kernel (processes 8 at a time) + 1 for tail
  ...
  // Setup params struct (not used for vrnd but required by signature)
  ...
  for (size_t i = 0; i < num_elements; i++) {
    symbolic_input.push_back(tm.mkConst(fp32, "x_" + std::to_string(i)));
  }
  ...
  // RVV uses vle32_v_f32m4 to load vectors
  for (size_t i = 0; i < num_elements; i++) {
    uintptr_t addr = reinterpret_cast<uintptr_t>(input.data() + i);
    ...
    // Vector memory for vle32 loads (m4 can hold more elements)
    std::vector<Term> chunk;
    for (size_t j = 0; j < 16; j++) {  // m4 can hold up to 16 elements
      size_t idx = i + j;
      if (idx < num_elements) {
    ...
    g_riscv_memory_f32m4[addr].push_back(vfloat32m4_t(g_symbolic_tm, chunk));
  }

  std::cout << "Testing XNNPACK f32_vrndd: NEON vs RISC-V equivalence" << std::endl;
  ...
  // Collect results from RISC-V (uses f32m4 stores)
  ...
  for (size_t i = 0; i < num_elements;) {
    ...
    // Try f32m4 first (what this kernel uses)
