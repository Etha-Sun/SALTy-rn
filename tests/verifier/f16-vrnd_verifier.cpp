...
// Forward declarations for f16 vrndd (round down/floor) kernels
extern "C" {
...
int main() {
  ...
  // f16-vrndd: Round down (floor) of float16
  // output[i] = floor(input[i])
  // NEON uses vrndmq_f16 (round toward -infinity)
  // RVV uses vfcvt with RDN rounding mode then converts back to float
  ...
  // Setup params struct (not used for vrndd but required by signatures)
  ...
  for (size_t i = 0; i < num_elements; i++) {
    ...
    // Constrain input range to avoid overflow when converting to int16
    // int16 range is -32768 to 32767
    // We need to ensure floor(x) fits in int16
    Term max_val = tm.mkFloatingPoint(5, 11, tm.mkBitVector(16, 0x6800));  // 2048.0h (safe range)
    Term min_val = tm.mkFloatingPoint(5, 11, tm.mkBitVector(16, 0xE800));  // -2048.0h
    Term in_range_upper = tm.mkTerm(Kind::FLOATINGPOINT_LT, {x_fp, max_val});
    Term in_range_lower = tm.mkTerm(Kind::FLOATINGPOINT_GT, {x_fp, min_val});
  ...
  std::cout << "Testing XNNPACK f16_vrndd: NEON vs RISC-V equivalence" << std::endl;
  std::cout << "Operation: output[i] = floor(input[i])" << std::endl;
  std::cout << "Number of elements: " << num_elements << std::endl;
  std::cout << "Batch size (bytes): " << batch << std::endl;

  // Run NEON implementation
  std::cout << "\nRunning NEON implementation..." << std::endl;
  xnn_f16_vrndd_ukernel__neonfp16arith_u16(
      batch,
  ...
  xnn_f16_vrndd_ukernel__rvv_u16(
      batch,
  ...
  // Note: Both implementations compute floor:
  // - NEON: vrndmq_f16 (direct floor in FP domain)
  // - RISC-V: vfcvt_x_f (float to int with RDN) + vfcvt_f_x (int back to float)
  // They should produce equivalent results for valid inputs within int16 range
  //
