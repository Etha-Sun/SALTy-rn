...
extern "C" {
void xnn_x16_packw_gemm_goi_ukernel_x8__neon_ld4lane_u16(
    size_t g,
    size_t nc,
    size_t kc,
    size_t nr,
...
int main() {
  ...
  const size_t kc = 19;     // Number of input channels
  ...
  // Allocate arrays
  std::vector<uint16_t> weights(weights_size, 0);
  std::vector<uint16_t> bias(bias_size, 0);
  std::vector<uint16_t> packed_weights_neon(packed_weights_size, 0);
  std::vector<uint16_t> packed_weights_riscv(packed_weights_size, 0);

  // Clear memory before each run
  SymbolicNEONHelpers::clearMemory();
  SymbolicRISCVHelpers::clearMemory();

  // Setup symbolic inputs (16-bit)
  std::vector<Term> symbolic_weights = createSymbolicArray(tm, weights_size, "w", 16);
  std::vector<Term> symbolic_bias = createSymbolicArray(tm, bias_size, "b", 16);

  // Populate NEON memory (unsigned 16-bit)
  SymbolicNEONHelpers::populateMemoryU16(weights.data(), symbolic_weights);
  SymbolicNEONHelpers::populateMemoryU16(bias.data(), symbolic_bias);

  // Populate RISC-V memory (unsigned 16-bit)
  SymbolicRISCVHelpers::populateMemoryU16(weights.data(), symbolic_weights);
  SymbolicRISCVHelpers::populateMemoryU16(bias.data(), symbolic_bias);

  std::cout << "Testing XNNPACK x16_packw: NEON vs RISC-V equivalence" << std::endl;
  ...
  xnn_x16_packw_gemm_goi_ukernel_x8__rvv_u16(
      g, nc, kc, nr, kr, sr,
      weights.data(), bias.data(), nullptr,
      packed_weights_riscv.data(), 0, nullptr);

  // Collect results from both architectures
  std::vector<Term> neon_elements = SymbolicNEONHelpers::collectResultsU16(packed_weights_neon.data(), packed_weights_size);
  std::vector<Term> riscv_elements = SymbolicRISCVHelpers::collectResultsU16(packed_weights_riscv.data());
