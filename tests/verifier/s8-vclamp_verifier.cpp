...
int main() {
  ...
  // Allocate arrays
  std::vector<int8_t> input(batch, 0);
  std::vector<int8_t> output_neon(batch, 0);
  std::vector<int8_t> output_riscv(batch, 0);
  ...
  SymbolicNEONHelpers::populateMemory8(input.data(), symbolic_input);

  // Populate RISC-V memory (signed 8-bit)
  SymbolicRISCVHelpers::populateMemory8(input.data(), symbolic_input);

  // Setup params struct with concrete test values
  struct xnn_s8_minmax_params params;
  params.scalar.min = -100;  // Clamp minimum
  params.scalar.max = 100;   // Clamp maximum

  std::cout << "Testing XNNPACK s8_vclamp: NEON vs RISC-V equivalence" << std::endl;
  std::cout << "Batch size: " << batch << " elements" << std::endl;
  std::cout << "Parameters:" << std::endl;
  std::cout << "  min: " << (int)params.scalar.min << std::endl;
  std::cout << "  max: " << (int)params.scalar.max << std::endl;

  g_current_params_ptr = nullptr;

  // Run NEON implementation
  std::cout << "\nRunning NEON implementation..." << std::endl;
  xnn_s8_vclamp_ukernel__neon_u64(batch * sizeof(int8_t), input.data(), output_neon.data(), &params);

  // Run RISC-V implementation
  std::cout << "Running RISC-V implementation..." << std::endl;
  xnn_s8_vclamp_ukernel__rvv_u64(batch * sizeof(int8_t), input.data(), output_riscv.data(), &params);

  // Collect results from both architectures
  std::vector<Term> neon_elements = SymbolicNEONHelpers::collectResults8(output_neon.data(), batch);
