...
// Forward declarations for f16 vhswish kernels
extern "C" {
...
int main() {
  ...
  // f16-vhswish: Hard Swish activation for float16
  // output[i] = x[i] * clamp(x[i] + 3, 0, 6) / 6
  // Equivalent to: output[i] = x[i] * min(max(x[i] + 3, 0), 6) / 6
  ...
  // Setup params struct (not used for vhswish but required by signatures)
  ...
  std::cout << "Testing XNNPACK f16_vhswish: NEON vs RISC-V equivalence" << std::endl;
  std::cout << "Operation: output[i] = x[i] * clamp(x[i] + 3, 0, 6) / 6" << std::endl;
  ...
  xnn_f16_vhswish_ukernel__rvv_u16(
      batch,
