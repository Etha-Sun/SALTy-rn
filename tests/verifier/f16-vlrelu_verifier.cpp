...
// Forward declarations for f16 vlrelu kernels
extern "C" {
...
int main() {
  ...
  // f16-vlrelu: Leaky ReLU for float16
  // output[i] = x[i] < 0 ? x[i] * slope : x[i]
  ...
  // Setup params struct with a symbolic slope
  // Use a concrete slope value for verification (e.g., 0.1)
  struct xnn_f16_lrelu_params params;
  _Float16 slope_value = (_Float16)0.1f;
  params.scalar.slope = xnn_float16_from_float(0.1f);
  ...
  // Populate RISC-V memory for input (using vle16_v_f16m8)
  // RVV loads directly as f16, so store fp16 terms
  {
    uintptr_t addr = reinterpret_cast<uintptr_t>(input_riscv.data());
    g_riscv_memory_f16m8[addr].push_back(vfloat16m8_t(g_symbolic_tm, symbolic_input_fp));
  }

  std::cout << "Testing XNNPACK f16_vlrelu: NEON vs RISC-V equivalence" << std::endl;
  std::cout << "Operation: output[i] = x[i] < 0 ? x[i] * slope : x[i]" << std::endl;
  std::cout << "Slope: 0.1" << std::endl;
  ...
  xnn_f16_vlrelu_ukernel__rvv_u16(
      batch,
  ...
  std::vector<Term> riscv_output_elements;
  {
    ...
    if (it != g_riscv_memory_f16m8.end() && !it->second.empty()) {
      const vfloat16m8_t& vec = it->second.back();
  ...
  // NEON outputs are bitvectors (from vst1q_u16), RVV outputs are fp16 (from vse16_v_f16m8)
