...
// Simplified 1x8 f16 igemm kernel for symbolic verification
// Based on xnn_f16_igemm_minmax_ukernel_1x8__neonfp16arith_ld64
    ...
    const struct xnn_f16_minmax_params* restrict params)
{
  ...
  do {
    // Load initial accumulator from weights (bias)
    float16x8_t vacc0x0 = vreinterpretq_f16_u16(vld1q_u16((const uint16_t*) w));
    w = (const xnn_float16*) ((const uint16_t*)w + 8);

    size_t p = ks;
    do {
      const uint16_t* restrict a0 = (const uint16_t*) a[0];
      assert(a0 != NULL);
      if (a0 != (const uint16_t*) zero) {
      ...
      for (; k >= 4 * sizeof(uint16_t); k -= 4 * sizeof(uint16_t)) {
        const float16x4_t va0 = vreinterpret_f16_u16(vld1_u16(a0));
        a0 += 4;

        const float16x8_t vb0c0 = vreinterpretq_f16_u16(vld1q_u16((const uint16_t*) w));
        w = (const xnn_float16*) ((const uint16_t*)w + 8);

        vacc0x0 = vfmaq_lane_f16(vacc0x0, vb0c0, va0, 0);

        const float16x8_t vb0c1 = vreinterpretq_f16_u16(vld1q_u16((const uint16_t*) w));
        w = (const xnn_float16*) ((const uint16_t*)w + 8);

        vacc0x0 = vfmaq_lane_f16(vacc0x0, vb0c1, va0, 1);

        const float16x8_t vb0c2 = vreinterpretq_f16_u16(vld1q_u16((const uint16_t*) w));
        w = (const xnn_float16*) ((const uint16_t*)w + 8);

        vacc0x0 = vfmaq_lane_f16(vacc0x0, vb0c2, va0, 2);

        const float16x8_t vb0c3 = vreinterpretq_f16_u16(vld1q_u16((const uint16_t*) w));
        w = (const xnn_float16*) ((const uint16_t*)w + 8);
      ...
      if (k != 0) {
        do {
          const float16x8_t va0 = vreinterpretq_f16_u16(vld1q_dup_u16(a0));
          a0 += 1;

          const float16x8_t vb0 = vreinterpretq_f16_u16(vld1q_u16((const uint16_t*) w));
          w = (const xnn_float16*) ((const uint16_t*)w + 8);
    ...
    if (nc >= 8) {
      ...
      if (nc & 4) {
        vst1_u16(c0, vreinterpret_u16_f16(vacc0));
        c0 += 4;
        vacc0 = vget_high_f16(vacc0x0);
      }
      if (nc & 2) {
        vst1_lane_u32((void*) c0, vreinterpret_u32_f16(vacc0), 0);
        c0 += 2;
