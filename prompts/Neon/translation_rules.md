...
<<<<<<< HEAD
- **Min/max families map directly under the frozen profile**: Under the frozen `AH = 0` profile, AArch64 `FMAX` / `FMIN` (the `vmax` / `vmin` / `vminnm` / `vmaxnm` NEON intrinsics) implement IEEE 754-2008 `maxNumber` / `minNumber` — quiet NaN is suppressed and the non-NaN operand is returned. RVV `vfmax` / `vfmin` (RVV 1.0 §14.11) use the same `maxNumber` / `minNumber` semantics. Therefore `vmax_fXX` → `vfmax_vv_fXXm*` and `vmin_fXX` → `vfmin_vv_fXXm*` is the exact lowering, with **no NaN fixup, no `vmfne`/`vmerge` dance, no NaN preservation.** Emitting a `vbool_t nan_mask = vmfne(acc, acc)` / `vmerge` sequence around the clamp is wrong: the NEON source does not preserve NaN, and injecting it back on the RVV side makes the two kernels diverge on any NaN-producing input (overflow, Inf × 0, etc.). Only consider fixups if the source operates under `AH = 1` (NaN-propagating) or the kernel explicitly documents NaN preservation. Compare families (`vceq`, `vcge`, `vcgt`, etc.) do not have signed-zero hazards; review only NaN behavior for those.
=======
...
>>>>>>> e3d5ca0053a79070386ceda9d5c78b8dd817f402
