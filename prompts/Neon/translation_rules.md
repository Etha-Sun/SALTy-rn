...
- **Min/max families need explicit scoping**: Keep `vmin` / `vmax` distinct from `vminnm` / `vmaxnm`. `vminnm` / `vmaxnm` map naturally to RVV `vfmin` / `vfmax`. For `vmin` / `vmax`, plain `vfmin` / `vfmax` is only exact when the source domain excludes NaNs and FPCR.AH matches. Under the frozen profile (`AH = 0`), negative zero compares less than positive zero on both ARM and RVV, so signed zero does not add a restriction. If NaN-equivalence is acceptable on NaN lanes, a valid lowering is: compute `vfmin` / `vfmax`, detect `isnan(a) || isnan(b)`, and overwrite those lanes with a fixed quiet NaN via `vmerge`. Compare families (`vceq`, `vcge`, `vcgt`, etc.) do not have signed-zero hazards; review only NaN behavior for those.
