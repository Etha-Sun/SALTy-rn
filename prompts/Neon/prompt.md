...
**Target:** Saturn — RVV 1.0, ELEN=64, VLEN={target_vlen}.
...
6. **LMUL for VLEN={target_vlen}:** Pick the largest LMUL (up to 8) that keeps the widest element type in the kernel within LMUL=8. At smaller VLEN, prefer higher LMUL so each stripmine iteration still processes a useful number of elements — VLMAX = LMUL × VLEN / SEW — and the per-iteration loop/issue overhead is amortized; e.g. at VLEN=128 a single-width f32 kernel should use e32m8 (VLMAX=32), not a smaller group. Keep the SEW/LMUL ratio constant across width stages (e.g., e8m2 → e16m4 → e32m8). Correctness is VLEN-agnostic regardless of this choice; LMUL only affects performance.
