...
_XNN_MIN_H = os.path.join(_ROOT, "src", "verification", "xnn_minimal.h")
...
def generate_multidim(neon_path, rvv_path, name, neon, rvv, spec, input_range=None) -> str:
    ...
    def nest(i):
        if i == len(dims):
            return ("        int _r = verify(" + dim_call + ", vlen, timeout);\n"
                    "        if (_r == 1) return 1;")
        d = dims[i]
    ...
    main_loop = nest(0)
    return f"""// AUTO-GENERATED by v2 emit.py — multi-dim harness ({name}).
#include "salt.hpp"
#include <cassert>
#include <cstdio>
#include <cstdlib>
...
def generate_harness(neon_path: str, rvv_path: str, name: str,
    ...
    if is_concrete:
        main_loop = f"""    for (size_t cfg = 0; cfg < {n_configs}; cfg++) {{
      for (size_t b = start; b <= end; b++) {{
        int r = verify(b, vlen, cfg, timeout);
        if (r == 1) return 1;   // counterexample — stop
        if (r == 2) break;      // timeout — larger batches only get harder; next config
      }}
    }}
    printf("{{\\"status\\":\\"ALL_VERIFIED\\",\\"start\\":%zu,\\"end\\":%zu,\\"configs\\":{n_configs}}}\\n", start, end);"""
    else:
        main_loop = f"""    for (size_t b = start; b <= end; b++) {{
        int r = verify(b, vlen, timeout);
        if (r == 1) return 1;   // counterexample — stop
        if (r == 2) break;      // timeout — larger batches only get harder
    }}
    printf("{{\\"status\\":\\"ALL_VERIFIED\\",\\"start\\":%zu,\\"end\\":%zu,\\"configs\\":1}}\\n", start, end);"""

    return f"""// AUTO-GENERATED by v2 emit.py — uniform harness (no Shape/profile).
#include "salt.hpp"
#include <cassert>
#include <cstdio>
#include <cstdlib>
