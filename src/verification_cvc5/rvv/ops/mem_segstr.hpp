...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e16_v_f16m1x2(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e16_v_f16m2x2(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e16_v_f16m4x2(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e16_v_f16mf2x2(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e16_v_f16mf4x2(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e32_v_f32m1x2(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e32_v_f32m2x2(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e32_v_f32m4x2(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e32_v_f32mf2x2(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e64_v_f64m1x2(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e64_v_f64m2x2(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlsseg2e64_v_f64m4x2(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e16_v_f16m1x3(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e16_v_f16m2x3(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e16_v_f16mf2x3(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e16_v_f16mf4x3(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e32_v_f32m1x3(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e32_v_f32m2x3(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e32_v_f32mf2x3(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e64_v_f64m1x3(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlsseg3e64_v_f64m2x3(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e16_v_f16m1x4(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e16_v_f16m2x4(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e16_v_f16mf2x4(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e16_v_f16mf4x4(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e32_v_f32m1x4(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e32_v_f32m2x4(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e32_v_f32mf2x4(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e64_v_f64m1x4(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlsseg4e64_v_f64m2x4(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlsseg5e16_v_f16m1x5(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,5> __riscv_vlsseg5e16_v_f16mf2x5(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlsseg5e16_v_f16mf4x5(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlsseg5e32_v_f32m1x5(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlsseg5e32_v_f32mf2x5(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlsseg5e64_v_f64m1x5(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlsseg6e16_v_f16m1x6(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,6> __riscv_vlsseg6e16_v_f16mf2x6(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlsseg6e16_v_f16mf4x6(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlsseg6e32_v_f32m1x6(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlsseg6e32_v_f32mf2x6(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlsseg6e64_v_f64m1x6(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlsseg7e16_v_f16m1x7(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,7> __riscv_vlsseg7e16_v_f16mf2x7(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlsseg7e16_v_f16mf4x7(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlsseg7e32_v_f32m1x7(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlsseg7e32_v_f32mf2x7(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlsseg7e64_v_f64m1x7(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlsseg8e16_v_f16m1x8(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,8> __riscv_vlsseg8e16_v_f16mf2x8(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlsseg8e16_v_f16mf4x8(const salt_float16* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlsseg8e32_v_f32m1x8(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlsseg8e32_v_f32mf2x8(const float* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlsseg8e64_v_f64m1x8(const double* base, ptrdiff_t bstride, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline void __riscv_vssseg2e16_v_f16m2x2(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
...
inline void __riscv_vssseg2e16_v_f16m4x2(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg2e16_v_f16mf2x2(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg2e16_v_f16mf4x2(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg2e32_v_f32m1x2(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg2e32_v_f32m2x2(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg2e32_v_f32m4x2(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg2e32_v_f32mf2x2(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg2e64_v_f64m1x2(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vssseg2e64_v_f64m2x2(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vssseg2e64_v_f64m4x2(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vssseg3e16_v_f16m1x3(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg3e16_v_f16m2x3(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg3e16_v_f16mf2x3(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg3e16_v_f16mf4x3(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg3e32_v_f32m1x3(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg3e32_v_f32m2x3(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg3e32_v_f32mf2x3(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg3e64_v_f64m1x3(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vssseg3e64_v_f64m2x3(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vssseg4e16_v_f16m1x4(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg4e16_v_f16m2x4(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg4e16_v_f16mf2x4(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg4e16_v_f16mf4x4(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg4e32_v_f32m1x4(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg4e32_v_f32m2x4(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg4e32_v_f32mf2x4(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg4e64_v_f64m1x4(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vssseg4e64_v_f64m2x4(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vssseg5e16_v_f16m1x5(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg5e16_v_f16mf2x5(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg5e16_v_f16mf4x5(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg5e32_v_f32m1x5(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg5e32_v_f32mf2x5(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg5e64_v_f64m1x5(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vssseg6e16_v_f16m1x6(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg6e16_v_f16mf2x6(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg6e16_v_f16mf4x6(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg6e32_v_f32m1x6(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg6e32_v_f32mf2x6(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg6e64_v_f64m1x6(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vssseg7e16_v_f16m1x7(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg7e16_v_f16mf2x7(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg7e16_v_f16mf4x7(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg7e32_v_f32m1x7(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg7e32_v_f32mf2x7(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg7e64_v_f64m1x7(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vssseg8e16_v_f16m1x8(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg8e16_v_f16mf2x8(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vssseg8e16_v_f16mf4x8(salt_float16* base, ptrdiff_t bstride, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vssseg8e32_v_f32m1x8(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vssseg8e32_v_f32mf2x8(float* base, ptrdiff_t bstride, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vssseg8e64_v_f64m1x8(double* base, ptrdiff_t bstride, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + i*(size_t)bstride + f*8, v.val[f].getElement(i), 64);
}
