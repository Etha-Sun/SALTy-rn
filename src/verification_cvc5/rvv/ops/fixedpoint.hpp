#pragma once
// RVV fixed-point family (cvc5): saturating/averaging add-sub, vsmul, scaling
...
inline Term _gfx_round_incr(TermManager& tm, Term value, size_t s, unsigned vxrm, size_t bits){
  Op ext_round = tm.mkOp(Kind::BITVECTOR_EXTRACT,
    {static_cast<uint32_t>(s-1), static_cast<uint32_t>(s-1)});
  Term round_bit = tm.mkTerm(ext_round, {value});
  Op zext = tm.mkOp(Kind::BITVECTOR_ZERO_EXTEND, {static_cast<uint32_t>(bits-1)});
  Term round_ext = tm.mkTerm(zext, {round_bit});
  ...
  if(vxrm==__RISCV_VXRM_RNE){
    ...
    Op ext_q0 = tm.mkOp(Kind::BITVECTOR_EXTRACT, {static_cast<uint32_t>(s), static_cast<uint32_t>(s)});
    Term q0 = tm.mkTerm(ext_q0, {value});                 // bit s = LSB of quotient
    Term q_odd = fold_eq(tm, q0, mk_bv_val(tm, 1, 1));
    Term tie;
    if(s>=2){
      Op ext_low = tm.mkOp(Kind::BITVECTOR_EXTRACT, {static_cast<uint32_t>(s-2), 0u});
      Term low = tm.mkTerm(ext_low, {value});
      Term low_zero = fold_eq(tm, low, mk_bv_val(tm, s-1, 0));
      tie = tm.mkTerm(Kind::OR, {tm.mkTerm(Kind::NOT, {low_zero}), q_odd});
    } else {
      tie = q_odd;
    }
    Term up = tm.mkTerm(Kind::AND, {fold_eq(tm, round_bit, mk_bv_val(tm,1,1)), tie});
...
inline Term _gfx_rshift(TermManager& tm, Term val, size_t s, size_t bits,
  ...
  Term shifted = is_signed ? tm.mkTerm(Kind::BITVECTOR_ASHR, {val, sh})
                           : fold_bvlshr(tm, val, sh);
  if(vxrm==__RISCV_VXRM_ROD){
    Op ext_trunc = tm.mkOp(Kind::BITVECTOR_EXTRACT, {static_cast<uint32_t>(s-1), 0u});
    Term truncated = tm.mkTerm(ext_trunc, {val});
...
inline RVVVector _gfx_sadd(const RVVVector& a, const RVVVector& b, size_t bits, bool is_signed, unsigned, size_t vl){
  ...
  Op ext = tm.mkOp(is_signed?Kind::BITVECTOR_SIGN_EXTEND:Kind::BITVECTOR_ZERO_EXTEND, {1u});
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term sum = fold_bvadd(tm, tm.mkTerm(ext,{a.getElement(i)}), tm.mkTerm(ext,{b.getElement(i)}));
...
inline RVVVector _gfx_sadd(const RVVVector& a, int64_t b, size_t bits, bool is_signed, unsigned, size_t vl){
  auto& tm=g_ctx->tm; size_t W=bits+1;
  Op ext = tm.mkOp(is_signed?Kind::BITVECTOR_SIGN_EXTEND:Kind::BITVECTOR_ZERO_EXTEND, {1u});
  Term be = tm.mkTerm(ext,{mk_bv_val(tm,bits,b)});
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term sum = fold_bvadd(tm, tm.mkTerm(ext,{a.getElement(i)}), be);
    r.push_back(is_signed?_gfx_sclip(tm,sum,W,bits):_gfx_uclip(tm,sum,W,bits));
  }
  return RVVVector(tm,bits,r);
}
...
inline RVVVector _gfx_ssub(const RVVVector& a, const RVVVector& b, size_t bits, bool is_signed, unsigned, size_t vl){
  ...
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term diff = fold_bvsub(tm, tm.mkTerm(ext,{a.getElement(i)}), tm.mkTerm(ext,{b.getElement(i)}));
...
inline RVVVector _gfx_ssub(const RVVVector& a, int64_t b, size_t bits, bool is_signed, unsigned, size_t vl){
  ...
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term diff = fold_bvsub(tm, tm.mkTerm(ext,{a.getElement(i)}), be);
    r.push_back(is_signed?_gfx_sclip(tm,diff,W,bits):_gfx_usub_clamp(tm,diff,bits));
  }
  return RVVVector(tm,bits,r);
}
...
inline RVVVector _gfx_aadd(const RVVVector& a, const RVVVector& b, size_t bits, bool is_signed, unsigned vxrm, size_t vl){
  auto& tm=g_ctx->tm; size_t W=bits+1;
  Op ext = tm.mkOp(is_signed?Kind::BITVECTOR_SIGN_EXTEND:Kind::BITVECTOR_ZERO_EXTEND, {1u});
  Op narrow = tm.mkOp(Kind::BITVECTOR_EXTRACT, {(uint32_t)(bits-1), 0u});
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term sum = fold_bvadd(tm, tm.mkTerm(ext,{a.getElement(i)}), tm.mkTerm(ext,{b.getElement(i)}));
    r.push_back(tm.mkTerm(narrow, {_gfx_rshift(tm, sum, 1, W, vxrm, is_signed)}));
  }
  return RVVVector(tm,bits,r);
}
...
inline RVVVector _gfx_asub(const RVVVector& a, const RVVVector& b, size_t bits, bool is_signed, unsigned vxrm, size_t vl){
  ...
  for(size_t i=0;i<vl;i++){
    Term diff = fold_bvsub(tm, tm.mkTerm(ext,{a.getElement(i)}), tm.mkTerm(ext,{b.getElement(i)}));
    r.push_back(tm.mkTerm(narrow, {_gfx_rshift(tm, diff, 1, W, vxrm, is_signed)}));
  }
  return RVVVector(tm,bits,r);
}
...
inline RVVVector _gfx_smul(const RVVVector& a, const RVVVector& b, size_t bits, bool, unsigned vxrm, size_t vl){
  ...
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term prod = fold_bvmul(tm, tm.mkTerm(sext,{a.getElement(i)}), tm.mkTerm(sext,{b.getElement(i)}));
...
inline RVVVector _gfx_smul(const RVVVector& a, int64_t b, size_t bits, bool, unsigned vxrm, size_t vl){
  ...
  std::vector<Term> r; r.reserve(vl);
  for(size_t i=0;i<vl;i++){
    Term prod = fold_bvmul(tm, tm.mkTerm(sext,{a.getElement(i)}), be);
    Term shifted = _gfx_rshift(tm, prod, bits-1, W, vxrm, true);
    r.push_back(_gfx_sclip(tm, shifted, W, bits));
  }
  return RVVVector(tm,bits,r);
...
inline RVVVector _gfx_ssr(const RVVVector& a, const RVVVector& sh, size_t bits, bool is_signed, unsigned vxrm, size_t vl){
  ...
  for(size_t i=0;i<vl;i++){
    Term s = tm.mkTerm(Kind::BITVECTOR_AND, {sh.getElement(i), mask});
...
inline RVVVector _gfx_nclip(const RVVVector& a, const RVVVector& sh, size_t bits, bool is_signed, unsigned vxrm, size_t vl){
  ...
  Op zext = tm.mkOp(Kind::BITVECTOR_ZERO_EXTEND, {(uint32_t)bits});
  for(size_t i=0;i<vl;i++){
    Term s = tm.mkTerm(Kind::BITVECTOR_AND, {tm.mkTerm(zext,{sh.getElement(i)}), mask});
...
inline RVVVector __riscv_vnclip_wx_i16m2(const RVVVector& op1, size_t op2, unsigned int vxrm, size_t vl){ return _gfx_nclip(op1, op2, 16, true, vxrm, vl); }
...
inline RVVVector __riscv_vnclip_wx_i8m1(const RVVVector& op1, size_t op2, unsigned int vxrm, size_t vl){ return _gfx_nclip(op1, op2, 8, true, vxrm, vl); }
