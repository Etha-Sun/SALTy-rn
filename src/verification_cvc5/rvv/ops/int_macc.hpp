#pragma once
// RVV integer multiply-accumulate (same-width) — generated (ns salt_cvc5).
...
inline Term _gmacc_lane(TermManager& tm, Term vd, Term x, Term y, MaccOp op){
  auto MUL=[&](Term p,Term q){return tm.mkTerm(Kind::BITVECTOR_MULT,{p,q});};
  auto ADD=[&](Term p,Term q){return tm.mkTerm(Kind::BITVECTOR_ADD,{p,q});};
  auto SUB=[&](Term p,Term q){return tm.mkTerm(Kind::BITVECTOR_SUB,{p,q});};
  switch(op){
...
inline RVVVector __riscv_vmacc_vx_i32m1(const RVVVector& vd, int32_t x, const RVVVector& y, size_t vl){ return _gmacc_vx(vd,(int64_t)x,y,32,M_MACC,vl); }
inline RVVVector __riscv_vmacc_vx_i32m2(const RVVVector& vd, int32_t x, const RVVVector& y, size_t vl){ return _gmacc_vx(vd,(int64_t)x,y,32,M_MACC,vl); }
inline RVVVector __riscv_vmacc_vx_i32m4(const RVVVector& vd, int32_t x, const RVVVector& y, size_t vl){ return _gmacc_vx(vd,(int64_t)x,y,32,M_MACC,vl); }
