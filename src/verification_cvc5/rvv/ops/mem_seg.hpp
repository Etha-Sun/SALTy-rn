...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16_v_f16m1x2(const salt_float16* base, size_t vl) {
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16_v_f16m2x2(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16_v_f16m4x2(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16_v_f16mf2x2(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16_v_f16mf4x2(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16ff_v_f16m1x2(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16ff_v_f16m2x2(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16ff_v_f16m4x2(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16ff_v_f16mf2x2(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e16ff_v_f16mf4x2(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32_v_f32m1x2(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32_v_f32m2x2(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32_v_f32m4x2(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32_v_f32mf2x2(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32ff_v_f32m1x2(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32ff_v_f32m2x2(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32ff_v_f32m4x2(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e32ff_v_f32mf2x2(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e64_v_f64m1x2(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e64_v_f64m2x2(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e64_v_f64m4x2(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e64ff_v_f64m1x2(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e64ff_v_f64m2x2(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,2> __riscv_vlseg2e64ff_v_f64m4x2(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,2> r;
  for (size_t f = 0; f < 2; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16_v_f16m1x3(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16_v_f16m2x3(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16_v_f16mf2x3(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16_v_f16mf4x3(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16ff_v_f16m1x3(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16ff_v_f16m2x3(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16ff_v_f16mf2x3(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e16ff_v_f16mf4x3(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e32_v_f32m1x3(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e32_v_f32m2x3(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e32_v_f32mf2x3(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e32ff_v_f32m1x3(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e32ff_v_f32m2x3(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e32ff_v_f32mf2x3(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e64_v_f64m1x3(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e64_v_f64m2x3(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e64ff_v_f64m1x3(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,3> __riscv_vlseg3e64ff_v_f64m2x3(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,3> r;
  for (size_t f = 0; f < 3; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16_v_f16m1x4(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16_v_f16m2x4(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16_v_f16mf2x4(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16_v_f16mf4x4(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16ff_v_f16m1x4(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16ff_v_f16m2x4(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16ff_v_f16mf2x4(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e16ff_v_f16mf4x4(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e32_v_f32m1x4(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e32_v_f32m2x4(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e32_v_f32mf2x4(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e32ff_v_f32m1x4(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e32ff_v_f32m2x4(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e32ff_v_f32mf2x4(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e64_v_f64m1x4(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e64_v_f64m2x4(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e64ff_v_f64m1x4(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,4> __riscv_vlseg4e64ff_v_f64m2x4(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,4> r;
  for (size_t f = 0; f < 4; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e16_v_f16m1x5(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e16_v_f16mf2x5(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e16_v_f16mf4x5(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e16ff_v_f16m1x5(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e16ff_v_f16mf2x5(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e16ff_v_f16mf4x5(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e32_v_f32m1x5(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e32_v_f32mf2x5(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e32ff_v_f32m1x5(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e32ff_v_f32mf2x5(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e64_v_f64m1x5(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,5> __riscv_vlseg5e64ff_v_f64m1x5(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,5> r;
  for (size_t f = 0; f < 5; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e16_v_f16m1x6(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e16_v_f16mf2x6(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e16_v_f16mf4x6(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e16ff_v_f16m1x6(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e16ff_v_f16mf2x6(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e16ff_v_f16mf4x6(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e32_v_f32m1x6(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e32_v_f32mf2x6(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e32ff_v_f32m1x6(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e32ff_v_f32mf2x6(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e64_v_f64m1x6(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,6> __riscv_vlseg6e64ff_v_f64m1x6(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,6> r;
  for (size_t f = 0; f < 6; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e16_v_f16m1x7(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e16_v_f16mf2x7(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e16_v_f16mf4x7(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e16ff_v_f16m1x7(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e16ff_v_f16mf2x7(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e16ff_v_f16mf4x7(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e32_v_f32m1x7(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e32_v_f32mf2x7(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e32ff_v_f32m1x7(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e32ff_v_f32mf2x7(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e64_v_f64m1x7(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,7> __riscv_vlseg7e64ff_v_f64m1x7(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,7> r;
  for (size_t f = 0; f < 7; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e16_v_f16m1x8(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e16_v_f16mf2x8(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e16_v_f16mf4x8(const salt_float16* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e16ff_v_f16m1x8(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e16ff_v_f16mf2x8(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e16ff_v_f16mf4x8(const salt_float16* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e32_v_f32m1x8(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e32_v_f32mf2x8(const float* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e32ff_v_f32m1x8(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e32ff_v_f32mf2x8(const float* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e64_v_f64m1x8(const double* base, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline VectorTuple<RVVVector,8> __riscv_vlseg8e64ff_v_f64m1x8(const double* base, size_t* new_vl, size_t vl) {
  auto& tm = g_ctx->tm; auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  VectorTuple<RVVVector,8> r;
  for (size_t f = 0; f < 8; f++) {
    std::vector<Term> lanes; lanes.reserve(vl);
...
inline void __riscv_vsseg2e16_v_f16m1x2(salt_float16* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
...
inline void __riscv_vsseg2e16_v_f16m2x2(salt_float16* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg2e16_v_f16m4x2(salt_float16* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg2e16_v_f16mf2x2(salt_float16* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg2e16_v_f16mf4x2(salt_float16* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg2e32_v_f32m1x2(float* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg2e32_v_f32m2x2(float* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg2e32_v_f32m4x2(float* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg2e32_v_f32mf2x2(float* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg2e64_v_f64m1x2(double* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vsseg2e64_v_f64m2x2(double* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vsseg2e64_v_f64m4x2(double* base, const VectorTuple<RVVVector,2>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 2; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*2+f)*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vsseg3e16_v_f16m1x3(salt_float16* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg3e16_v_f16m2x3(salt_float16* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg3e16_v_f16mf2x3(salt_float16* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg3e16_v_f16mf4x3(salt_float16* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg3e32_v_f32m1x3(float* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg3e32_v_f32m2x3(float* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg3e32_v_f32mf2x3(float* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg3e64_v_f64m1x3(double* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vsseg3e64_v_f64m2x3(double* base, const VectorTuple<RVVVector,3>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 3; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*3+f)*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vsseg4e16_v_f16m1x4(salt_float16* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg4e16_v_f16m2x4(salt_float16* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg4e16_v_f16mf2x4(salt_float16* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg4e16_v_f16mf4x4(salt_float16* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg4e32_v_f32m1x4(float* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg4e32_v_f32m2x4(float* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg4e32_v_f32mf2x4(float* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg4e64_v_f64m1x4(double* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*8, v.val[f].getElement(i), 64);
}
inline void __riscv_vsseg4e64_v_f64m2x4(double* base, const VectorTuple<RVVVector,4>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 4; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*4+f)*8, v.val[f].getElement(i), 64);
}
...
inline void __riscv_vsseg5e16_v_f16m1x5(salt_float16* base, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*5+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg5e16_v_f16mf2x5(salt_float16* base, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*5+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg5e16_v_f16mf4x5(salt_float16* base, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*5+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg5e32_v_f32m1x5(float* base, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*5+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg5e32_v_f32mf2x5(float* base, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*5+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg5e64_v_f64m1x5(double* base, const VectorTuple<RVVVector,5>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 5; f++)
...
inline void __riscv_vsseg6e16_v_f16m1x6(salt_float16* base, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*6+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg6e16_v_f16mf2x6(salt_float16* base, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*6+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg6e16_v_f16mf4x6(salt_float16* base, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*6+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg6e32_v_f32m1x6(float* base, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*6+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg6e32_v_f32mf2x6(float* base, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*6+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg6e64_v_f64m1x6(double* base, const VectorTuple<RVVVector,6>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 6; f++)
...
inline void __riscv_vsseg7e16_v_f16m1x7(salt_float16* base, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*7+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg7e16_v_f16mf2x7(salt_float16* base, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*7+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg7e16_v_f16mf4x7(salt_float16* base, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*7+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg7e32_v_f32m1x7(float* base, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*7+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg7e32_v_f32mf2x7(float* base, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*7+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg7e64_v_f64m1x7(double* base, const VectorTuple<RVVVector,7>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 7; f++)
...
inline void __riscv_vsseg8e16_v_f16m1x8(salt_float16* base, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*8+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg8e16_v_f16mf2x8(salt_float16* base, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*8+f)*2, v.val[f].getElement(i), 16);
}
inline void __riscv_vsseg8e16_v_f16mf4x8(salt_float16* base, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*8+f)*2, v.val[f].getElement(i), 16);
}
...
inline void __riscv_vsseg8e32_v_f32m1x8(float* base, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*8+f)*4, v.val[f].getElement(i), 32);
}
inline void __riscv_vsseg8e32_v_f32mf2x8(float* base, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
    for (size_t i = 0; i < vl; i++) b.storeScalar(o + (i*8+f)*4, v.val[f].getElement(i), 32);
}
...
inline void __riscv_vsseg8e64_v_f64m1x8(double* base, const VectorTuple<RVVVector,8>& v, size_t vl) {
  auto& b = g_ctx->findBuffer(base); size_t o = b.ptrToByteOffset(base);
  for (size_t f = 0; f < 8; f++)
