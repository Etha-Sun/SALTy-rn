...
inline int8x8_t vqadd_s8(const int8x8_t& a, const int8x8_t& b) {
    ...
    for (size_t i = 0; i < 8; i++) _nres[i] = _nsat_sadd(tm, a.getLane(i), b.getLane(i), 8);
...
inline int32x4_t vqaddq_s32(const int32x4_t& a, const int32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nsat_sadd(tm, a.getLane(i), b.getLane(i), 32);
...
inline int16x8_t vqsubq_s16(const int16x8_t& a, const int16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nsat_ssub(tm, a.getLane(i), b.getLane(i), 16);
...
inline int16x4_t vqrdmulh_s16(const int16x4_t& a, const int16x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nqd_mulh(tm, a.getLane(i), b.getLane(i), 16, true);
    return int16x4_t(tm, _nres);
}
inline int32x2_t vqrdmulh_s32(const int32x2_t& a, const int32x2_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 2> _nres;
    for (size_t i = 0; i < 2; i++) _nres[i] = _nqd_mulh(tm, a.getLane(i), b.getLane(i), 32, true);
    return int32x2_t(tm, _nres);
}
inline int32x4_t vqrdmulhq_s32(const int32x4_t& a, const int32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nqd_mulh(tm, a.getLane(i), b.getLane(i), 32, true);
    return int32x4_t(tm, _nres);
}
