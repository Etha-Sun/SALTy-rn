...
inline float32x4_t vmulq_n_f32(const float32x4_t& a, float32_t b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_mul(tm,lane_to_fp(tm, a.getLane(i), 32),mk_fpc32(tm, b)), 32);
...
inline float32x4_t vmulq_lane_f32(const float32x4_t& a, const float32x2_t& v, const int lane) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
...
inline float32x4_t vminnmq_f32(const float32x4_t& a, const float32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_minnm(tm,lane_to_fp(tm, a.getLane(i), 32),lane_to_fp(tm, b.getLane(i), 32)), 32);
    return float32x4_t(tm, _nres);
}
...
inline float16x8_t vminnmq_f16(const float16x8_t& a, const float16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = fp_to_lane(tm, nfp_minnm(tm,lane_to_fp(tm, a.getLane(i), 16),lane_to_fp(tm, b.getLane(i), 16)), 16);
    return float16x8_t(tm, _nres);
}
...
inline float32x4_t vmaxnmq_f32(const float32x4_t& a, const float32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_maxnm(tm,lane_to_fp(tm, a.getLane(i), 32),lane_to_fp(tm, b.getLane(i), 32)), 32);
    return float32x4_t(tm, _nres);
}
...
inline float16x8_t vmaxnmq_f16(const float16x8_t& a, const float16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = fp_to_lane(tm, nfp_maxnm(tm,lane_to_fp(tm, a.getLane(i), 16),lane_to_fp(tm, b.getLane(i), 16)), 16);
    return float16x8_t(tm, _nres);
}
...
inline float32x4_t vrecpsq_f32(const float32x4_t& a, const float32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_sub(tm,mk_fpc(tm,2.0,32),nfp_mul(tm,lane_to_fp(tm, a.getLane(i), 32),lane_to_fp(tm, b.getLane(i), 32))), 32);
    return float32x4_t(tm, _nres);
}
...
inline float32x4_t vnegq_f32(const float32x4_t& a) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_neg(tm,lane_to_fp(tm, a.getLane(i), 32)), 32);
    return float32x4_t(tm, _nres);
}
...
inline float16x8_t vnegq_f16(const float16x8_t& a) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = fp_to_lane(tm, nfp_neg(tm,lane_to_fp(tm, a.getLane(i), 16)), 16);
    return float16x8_t(tm, _nres);
}
...
inline float32x4_t vsqrtq_f32(const float32x4_t& a) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_sqrt(tm,lane_to_fp(tm, a.getLane(i), 32)), 32);
    return float32x4_t(tm, _nres);
}
...
inline float32x4_t vrecpeq_f32(const float32x4_t& a) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_div(tm,mk_fpc(tm,1.0,32),lane_to_fp(tm, a.getLane(i), 32)), 32);
    return float32x4_t(tm, _nres);
}
...
inline float16x4_t vfma_f16(const float16x4_t& a, const float16x4_t& b, const float16x4_t& c) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_fma(tm,lane_to_fp(tm, b.getLane(i), 16),lane_to_fp(tm, c.getLane(i), 16),lane_to_fp(tm, a.getLane(i), 16)), 16);
    return float16x4_t(tm, _nres);
}
...
inline float32x4_t vfmsq_f32(const float32x4_t& a, const float32x4_t& b, const float32x4_t& c) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = fp_to_lane(tm, nfp_fma(tm,nfp_neg(tm,lane_to_fp(tm, b.getLane(i), 32)),lane_to_fp(tm, c.getLane(i), 32),lane_to_fp(tm, a.getLane(i), 32)), 32);
    return float32x4_t(tm, _nres);
}
...
inline float32x2_t vmla_f32(const float32x2_t& a, const float32x2_t& b, const float32x2_t& c) {
    auto& tm = g_ctx->tm;
    std::array<Term, 2> _nres;
    for (size_t i = 0; i < 2; i++) _nres[i] = fp_to_lane(tm, nfp_add(tm,lane_to_fp(tm, a.getLane(i), 32),nfp_mul(tm,lane_to_fp(tm, b.getLane(i), 32),lane_to_fp(tm, c.getLane(i), 32))), 32);
    return float32x2_t(tm, _nres);
}
...
inline float32x4_t vmlaq_lane_f32(const float32x4_t& a, const float32x4_t& b, const float32x2_t& v, const int lane) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
...
inline uint32x4_t vceqq_f32(const float32x4_t& a, const float32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = mask_from(tm, nfp_eq(tm, lane_to_fp(tm, a.getLane(i), 32), lane_to_fp(tm, b.getLane(i), 32)), 32);
...
inline uint32x4_t vcleq_f32(const float32x4_t& a, const float32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = mask_from(tm, nfp_le(tm, lane_to_fp(tm, a.getLane(i), 32), lane_to_fp(tm, b.getLane(i), 32)), 32);
    return uint32x4_t(tm, _nres);
}
...
inline float32x4_t vsetq_lane_f32(float32_t a, const float32x4_t& v, const int lane) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t j = 0; j < 4; j++) _nres[j] = v.getLane(j);
    _nres[lane] = fp_rawbits32(tm,a);
