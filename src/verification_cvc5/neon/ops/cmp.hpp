...
inline uint8x8_t vceq_s8(const int8x8_t& a, const int8x8_t& b) {
    auto& tm = g_ctx->tm;
...
inline uint8x16_t vceqq_s8(const int8x16_t& a, const int8x16_t& b) {
    auto& tm = g_ctx->tm;
...
inline uint16x4_t vceq_s16(const int16x4_t& a, const int16x4_t& b) {
    auto& tm = g_ctx->tm;
...
inline uint16x8_t vceqq_s16(const int16x8_t& a, const int16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = mask_from(tm, cmp_eq(tm, a.getLane(i), b.getLane(i)), 16);
    return uint16x8_t(tm, _nres);
}
inline uint32x2_t vceq_s32(const int32x2_t& a, const int32x2_t& b) {
    auto& tm = g_ctx->tm;
...
inline uint32x4_t vceqq_s32(const int32x4_t& a, const int32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
...
inline uint16x8_t vcgtq_s16(const int16x8_t& a, const int16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = mask_from(tm, cmp_sgt(tm, a.getLane(i), b.getLane(i)), 16);
    return uint16x8_t(tm, _nres);
}
...
inline uint32x4_t vcgtq_s32(const int32x4_t& a, const int32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = mask_from(tm, cmp_sgt(tm, a.getLane(i), b.getLane(i)), 32);
    return uint32x4_t(tm, _nres);
}
