...
inline int8x8_t vhadd_s8(const int8x8_t& a, const int8x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, true, false, false);
    return int8x8_t(tm, _nres);
}
inline int8x16_t vhaddq_s8(const int8x16_t& a, const int8x16_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 16> _nres;
    for (size_t i = 0; i < 16; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, true, false, false);
    return int8x16_t(tm, _nres);
}
inline int16x4_t vhadd_s16(const int16x4_t& a, const int16x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, true, false, false);
    return int16x4_t(tm, _nres);
}
inline int16x8_t vhaddq_s16(const int16x8_t& a, const int16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, true, false, false);
    return int16x8_t(tm, _nres);
}
inline int32x2_t vhadd_s32(const int32x2_t& a, const int32x2_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 2> _nres;
    for (size_t i = 0; i < 2; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, true, false, false);
    return int32x2_t(tm, _nres);
}
inline int32x4_t vhaddq_s32(const int32x4_t& a, const int32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, true, false, false);
    return int32x4_t(tm, _nres);
}
inline uint8x8_t vhadd_u8(const uint8x8_t& a, const uint8x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, false, false, false);
    return uint8x8_t(tm, _nres);
}
inline uint8x16_t vhaddq_u8(const uint8x16_t& a, const uint8x16_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 16> _nres;
    for (size_t i = 0; i < 16; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, false, false, false);
    return uint8x16_t(tm, _nres);
}
inline uint16x4_t vhadd_u16(const uint16x4_t& a, const uint16x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, false, false, false);
    return uint16x4_t(tm, _nres);
}
inline uint16x8_t vhaddq_u16(const uint16x8_t& a, const uint16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, false, false, false);
    return uint16x8_t(tm, _nres);
}
inline uint32x2_t vhadd_u32(const uint32x2_t& a, const uint32x2_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 2> _nres;
    for (size_t i = 0; i < 2; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, false, false, false);
    return uint32x2_t(tm, _nres);
}
inline uint32x4_t vhaddq_u32(const uint32x4_t& a, const uint32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, false, false, false);
    return uint32x4_t(tm, _nres);
}
inline int8x8_t vrhadd_s8(const int8x8_t& a, const int8x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, true, false, true);
    return int8x8_t(tm, _nres);
}
inline int8x16_t vrhaddq_s8(const int8x16_t& a, const int8x16_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 16> _nres;
    for (size_t i = 0; i < 16; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, true, false, true);
    return int8x16_t(tm, _nres);
}
inline int16x4_t vrhadd_s16(const int16x4_t& a, const int16x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, true, false, true);
    return int16x4_t(tm, _nres);
}
inline int16x8_t vrhaddq_s16(const int16x8_t& a, const int16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, true, false, true);
    return int16x8_t(tm, _nres);
}
inline int32x2_t vrhadd_s32(const int32x2_t& a, const int32x2_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 2> _nres;
    for (size_t i = 0; i < 2; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, true, false, true);
    return int32x2_t(tm, _nres);
}
inline int32x4_t vrhaddq_s32(const int32x4_t& a, const int32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, true, false, true);
    return int32x4_t(tm, _nres);
}
inline uint8x8_t vrhadd_u8(const uint8x8_t& a, const uint8x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, false, false, true);
    return uint8x8_t(tm, _nres);
}
inline uint8x16_t vrhaddq_u8(const uint8x16_t& a, const uint8x16_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 16> _nres;
    for (size_t i = 0; i < 16; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 8, false, false, true);
    return uint8x16_t(tm, _nres);
}
inline uint16x4_t vrhadd_u16(const uint16x4_t& a, const uint16x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, false, false, true);
    return uint16x4_t(tm, _nres);
}
inline uint16x8_t vrhaddq_u16(const uint16x8_t& a, const uint16x8_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 16, false, false, true);
    return uint16x8_t(tm, _nres);
}
inline uint32x2_t vrhadd_u32(const uint32x2_t& a, const uint32x2_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 2> _nres;
    for (size_t i = 0; i < 2; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, false, false, true);
    return uint32x2_t(tm, _nres);
}
inline uint32x4_t vrhaddq_u32(const uint32x4_t& a, const uint32x4_t& b) {
    auto& tm = g_ctx->tm;
    std::array<Term, 4> _nres;
    for (size_t i = 0; i < 4; i++) _nres[i] = _nhalf(tm, a.getLane(i), b.getLane(i), 32, false, false, true);
    return uint32x4_t(tm, _nres);
}
