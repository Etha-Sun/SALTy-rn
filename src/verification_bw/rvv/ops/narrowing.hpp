#pragma once
// RVV narrowing shifts/convert (source 2*SEW -> result SEW) — generated (ns salt).
...
inline RVVVector _gn_sh_wv(const RVVVector& a,const RVVVector& sh,size_t sew,bool arith,size_t vl){
  ...
  for(size_t i=0;i<vl;i++){
    ...
    Term m=tm.mk_term(Kind::BV_AND,{s,mk_bv_val(tm,w,(int64_t)(w-1))});
    Term v=tm.mk_term(arith?Kind::BV_ASHR:Kind::BV_SHR,{a.getElement(i),m});
...
inline RVVVector __riscv_vncvt_x_x_w_u8m1(const RVVVector& a, size_t vl){ return _gn_trunc(a,8,vl); }
...
inline RVVVector __riscv_vnsrl_wx_u16m1(const RVVVector& a, size_t shift, size_t vl){ return _gn_sh_wx(a,shift,16,false,vl); }
...
inline RVVVector __riscv_vnsrl_wx_u16mf2(const RVVVector& a, size_t shift, size_t vl){ return _gn_sh_wx(a,shift,16,false,vl); }
