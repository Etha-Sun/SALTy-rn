...
inline int16x8_t vcvtnq_s16_f16(const float16x8_t& a) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = fp_to_sint(tm, lane_to_fp(tm,a.getLane(i),16), rm_of(tm,0), 16);
    return int16x8_t(tm, _nres);
}
...
inline float16x8_t vcvtq_f16_s16(const int16x8_t& a) {
    auto& tm = g_ctx->tm;
    std::array<Term, 8> _nres;
    for (size_t i = 0; i < 8; i++) _nres[i] = fp_to_lane(tm, sint_to_fp(tm,a.getLane(i),rm_of(tm,0),16), 16);
