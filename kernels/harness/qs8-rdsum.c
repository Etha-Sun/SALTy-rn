...
#include "qs8-rdsum/gen/qs8-rdsum-minmax-fp32-scalar-u1-acc1.c"
...
static int8_t input[ROWS * CHANNELS];
static int8_t zero[CHANNELS];        // zero-padding row for the 7p7x tail
static int32_t out_rvv[CHANNELS];
static int32_t gold[CHANNELS];

int full_is_equal(const int32_t* x, const int32_t* y) {
    for (size_t i = 0; i < CHANNELS; i++) {
        if (x[i] != y[i]) {
            printf("Mismatch at channel %zu: got %d, expected %d\n", i, x[i], y[i]);
            return 0;
        }
    }
    return 1;
...
int main() {
    ...
    xnn_qs8_rdsum_ukernel_7p7x__scalar_u4(ROWS, CHANNELS, input, input_stride, zero, gold, &params);
