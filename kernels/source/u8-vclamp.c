...
void test_neon(
    ...
    const uint8_t* input,
    uint8_t* output,
    const struct xnn_u8_minmax_params* restrict params) XNN_OOB_READS
{
  assert(batch != 0);
  ...
  const uint8x16_t voutput_max = vdupq_n_u8(params->scalar.max);
  const uint8x16_t voutput_min = vdupq_n_u8(params->scalar.min);
